...
country_transaction <- retail %>% group_by(Country) %>% summarise(TotalValue = sum(TransactionValue, na.rm = TRUE)) %>%  filter(TotalValue > 130000) %>% arrange(desc(TotalValue))
...
retail %>% group_by(Invoice_Day_Week) %>% summarise(TotalValue = sum(TransactionValue, na.rm = TRUE))%>%
summarise(TransactionCount = n()) %>%
mutate(Percentage = 100 * TransactionCount / sum(TransactionCount)) %>%
arrange(desc(Percentage))
...
# Highest number of transactions (count of unique invoices)
...
```{r}
# Count number of unique customers (excluding missing values)
